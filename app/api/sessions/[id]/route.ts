...
async function feedback(s:Session,a:Attempt){
 ...
 const result=await ai<Feedback>(coachingRules,{scene:s.scene,goal:s.goal,history:history(s),currentOpponent:s.turns.at(-1)?.opponent,actualAnswer:a.text},feedbackSchema);
